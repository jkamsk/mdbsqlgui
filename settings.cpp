...
void Settings::read()
{
    if (Self == 0L)
    {
        qDebug(QString("Failed read config json file, Yout need call Settings:init() function").toStdString().c_str(),"%s");
        return;
    }
    ...
    {
        qDebug(QString("Failed read config json file, %1").arg(Self->Config).toStdString().c_str(),"%s");
        return;
    }
    ...
    {
        qDebug(QString("Parsing JSON config failed, %1").arg(Self->Config).toStdString().c_str(),"%s");
        return;
    }
    ...
    if (cf.contains("Connections"))
       ...
       qDebug(QString("Connections section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");

    if (cf.contains("TableTemplates"))
        TableTemplates = cf["TableTemplates"].toObject();
    else
       qDebug(QString("TableTemplates section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");

    if (cf.contains("Options"))
        Options = cf["Options"].toObject();
    else
       qDebug(QString("Options section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");

    if (cf.contains("Queries"))
        Queries = cf["Queries"].toObject();
    else
       qDebug(QString("Queries section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");

    if (cf.contains("Engines"))
        Engines = cf["Engines"].toArray();
    else
       qDebug(QString("Engines section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");

    if (cf.contains("ColumnTypes"))
        ColumnTypes = cf["ColumnTypes"].toArray();
    else
       qDebug(QString("ColumnTypes section not found, %1").arg(Self->Config).toStdString().c_str(),"%s");
}
void Settings::write()
{
    if (Self == 0L)
    {
        qDebug(QString("Failed write to config json file, Yout need call Settings:init() function").toStdString().c_str(),"%s");
        return;
    }
    QFile f(Self->Config);
    if (!f.open(QFile::WriteOnly))
    {
        qDebug(QString("Failed write to config json file, %1").arg(Self->Config).toStdString().c_str(),"%s");
        return;
    }
